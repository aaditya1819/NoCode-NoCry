...
import { SmartphoneIcon } from 'lucide-react' // Added missing import
...
const Projects = () => {
  ...
  const [isGenerating, setIsGenerating] = useState(false) // Fixed typo
  ...
  const togglePublish = async () => {
    try {
      ...
      toast.success(data.message)
      await fetchProject() // Refresh project state to show correct status
    } catch (error: any) {
      toast.error(error?.response?.data?.message || error.message)
    }
  }
  ...
  return project ? (
    <div className='flex flex-col h-screen w-full bg-gray-900 text-white'>
      ...
      <div className='flex-1 flex overflow-auto'>
        <div className={`${isMenuOpen ? 'max-sm:hidden' : ''} sm:w-96 h-full`}>
